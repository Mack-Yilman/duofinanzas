...
export function toIncome(page: any): Income {
  return {
    ...
    type: IncomeTypeSchema.safeParse(getSelect(page, 'type')).success ? IncomeTypeSchema.parse(getSelect(page, 'type')) : 'salary',
    period: IncomePeriodSchema.safeParse(getSelect(page, 'period')).success ? IncomePeriodSchema.parse(getSelect(page, 'period')) : 'monthly',
